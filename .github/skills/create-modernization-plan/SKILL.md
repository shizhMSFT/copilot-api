...
4. **Generate plan and tasks**: Generate plan.md and tasks.json using the appropriate templates:
    ...
    **Integration Test Task Rules**: Add an integration test task when EITHER of these conditions is met:
     ...
     **Baseline Task Rules**: A setupBaseline task is **mandatory whenever an integrationTest task is included** in the plan.
     - **Ordering**: setupBaseline must come before any upgrade or transform task, so the baseline is captured against the unmodified codebase. The only task that may precede it is `infrastructure` (a provisioning task, not a coding task). The resulting execution order is: `infrastructure` (if present) → `setupBaseline` → upgrade/transform → integrationTest.
